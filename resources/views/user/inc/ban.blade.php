<div class="col-12 mt-3 text-break scroll" style="background: rgb(246, 240, 204);">
    <div id="carouselExampleDark" data-bs-touch="true" class="carousel carousel-dark slide mx-0" data-bs-interval="false" data-bs-ride="carousel">
        ...
        <div class="carousel-inner pb-4 px-5 pt-3 overflow-auto">
            <div class="carousel-item active pb-2 text-break">
                @if (!is_null($model['forums_block']) && $model['forums_block']->count() != 0 && $model['user']['role_id'] >= $model['user_inf']['role_id'])
                    <h5>Блокировка пользователей</h5>
                    <form method='post' action='{{ url('/u/'.$model['user_inf']['id'].'/forum_block') }}'>
                        @csrf
                        <div class="row">
                            <div class="col-lg-4 col-md-5 col-12 my-1" style="min-height:218px !important">
                                <p class="fw-bold">Ветки форумов</p>
                                <div class="overflow-auto" style="max-height: 150px; position: relative;">
                                    @foreach ($model['forums_block'] as $forum)
                                        <div class="form-check p-0 new-tema">
                                            @if ($forum->section_id == 7)
                                                ...
                                                <label class="btn btn-outline-primary p-0" style="color: #bd0000" for="{{ $forum['id'] }}">
                                                    <span class="d-sm-inline">{{ $forum['title'] }}</span>
                                                </label>
                                            @elseif ($forum->id == 16 || $forum->id == 17 || $forum->id == 39 || $forum->id == 40 || $forum->section_id == 5)
                                                <input type="radio" class="btn-check" name="check[]" id="{{ $forum['id'] }}" value="{{ $forum['id'] }}">
                                                <label class="btn btn-outline-primary p-0" style="color: #00299d" for="{{ $forum['id'] }}">
                                                    <span class="d-sm-inline">{{ $forum['title'] }}</span>
                                                </label>
                                            @else
                                                <input type="radio" class="btn-check" name="check[]" id="{{ $forum['id'] }}" value="{{ $forum['id'] }}">
                                                <label class="btn btn-outline-primary p-0" style="color: #160f0a" for="{{ $forum['id'] }}">
                                                    <span class="d-sm-inline">{{ $forum['title'] }}</span>
                                                </label>
                                            @endif
                                        </div>
                            ...
                            <div class="col-lg-8 col-md-7 col-12  my-1" style="min-height:218px !important">
                                ...
                                <textarea type="text" name="text" id="text" title="причина бана" class="border border-1 input-text mb-2" style="height: 50px !important; "></textarea>
            ...
            <div class="carousel-item pb-2 text-break">
                @if (!is_null($model['forums_block']) && $model['forums_block']->count() != 0 && $model['user']['role_id'] >= $model['user_inf']['role_id'])
                    <h5>Блокировка пользователей</h5>
                    <form method='post' action='{{ url('/u/'.$model['user_inf']['id'].'/section_block') }}'>
                        @csrf
                        <div class="row">
                            <div class="col-lg-4 col-md-5 col-12 scroll my-1" style="min-height:218px !important">
                                ...
                                <div class="overflow-auto" style="max-height: 150px; position: relative;">
                                    @foreach ($model['sections_block'] as $section)
                                        <div class="form-check p-0 new-tema">
                                            @if ($section->id == 7)
                                                <input type="radio" class="btn-check" name="check[]" id="{{ $section['id'] }}" value="{{ $section['id'] }}">
                                                <label class="btn btn-outline-primary p-0" style="color: #bd0000" for="{{ $section['id'] }}">
                                                    <span class="d-sm-inline">{{ $section['title'] }}</span>
                                                </label>
                                            @elseif ($section->id == 5)
                                                <input type="radio" class="btn-check" name="check[]" id="{{ $section['id'] }}" value="{{ $section['id'] }}">
                                                <label class="btn btn-outline-primary p-0" style="color: #00299d" for="{{ $section['id'] }}">
                                                    <span class="d-sm-inline">{{ $section['title'] }}</span>
                                                </label>
                                            @else
                                                <input type="radio" class="btn-check" name="check[]" id="{{ $section['id'] }}" value="{{ $section['id'] }}">
                                                <label class="btn btn-outline-primary p-0" style="color: #160f0a" for="{{ $section['id'] }}">
                                                    <span class="d-sm-inline">{{ $section['title'] }}</span>
                                                </label>
                                            @endif
                                        </div>
            ...
            <div class="carousel-item pb-2 text-break">
                @if (!is_null($model['forums_block']) && $model['forums_block']->count() != 0 && $model['user']['role_id'] >= $model['user_inf']['role_id'])
                    <h5>Блокировка пользователей</h5>
                    <form method='post' action='{{ url('/u/'.$model['user_inf']['id'].'/topic_block') }}'>
                        @csrf
                        <div class="row">
                            <div class="col-12 my-1" style="min-height:218px !important">
                                <p class="fw-bold">Темы форумов</p>
                                <div class="col-auto d-flex justify-content-start pe-0"><span>Id темы:&nbsp;</span><input type="number" name="d" id="only_num" title="дней бана" class="border border-1 input-text-number mb-2 d-inline" onkeypress="if(event.keyCode == 13) return false;"></div>
                                <p class="fw-bold">Причина бана:</p>
                                <textarea type="text" name="text" id="text" title="причина бана" class="border border-1 input-text mb-2" style="height: 29px !important; "></textarea>
                                <p class="fw-bold">Время:</p>
                                <div class="row">
                                    <div class="col-auto d-flex justify-content-start pe-0"><span>дни:&nbsp;</span><input type="number" name="d" id="only_num" title="дней бана" class="border border-1 input-text-number mb-2 d-inline" onkeypress="if(event.keyCode == 13) return false;"></div>
                                    <div class="col-auto d-flex justify-content-start pe-0"><span>часы:&nbsp;</span><input type="number" name="h" id="only_num" title="часов бана" class="border border-1 input-text-number mb-2 d-inline" onkeypress="if(event.keyCode == 13) return false;"></div>
