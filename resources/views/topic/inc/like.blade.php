    ...
    <span
        ...
        @if (is_null($post['like']) || $post['like']->action == 'like')
            <a href="{{ url('/p/' . $post['id'] . '/dislike/' . $model['pagination']['page']) }}" style="text-decoration: none !important;">
                <i class="fa-regular fa-thumbs-down" style="color: #700000"></i>
            </a>
        @elseif ($post['like']->action == 'dislike')
            <a href="{{ url('/p/' . $post['id'] . '/dislikem/' . $model['pagination']['page']) }}" style="text-decoration: none !important;">
                <i class="fa-solid fa-thumbs-down" style="color: #700000"></i>
            </a>
        @endif
        <span class="ms-1" style="color: #700000">{{ $post['DATA']->dislike }}</span>
    </span>
@else
    <span
        @if (!is_null($post['DATA']->like_name)) data-bs-toggle="popover" data-bs-trigger="hover" data-bs-placement="top" data-bs-content="
